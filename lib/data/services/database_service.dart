...
class DatabaseService {
  ...
  Future<Map<String, dynamic>> robleCreate(
    ...
    final rowIdField = '${resolvedTableName}_id';
    if (!payload.containsKey(rowIdField) || payload[rowIdField] == null) {
      payload[rowIdField] = _uuid.v4();
    } else {
      payload[rowIdField] = payload[rowIdField].toString();
    }

    final row = await roble.create(resolvedTableName, payload);

    final hasPrimaryKey = row.containsKey('_id') || row.containsKey('id');
  ...
  Future<List<Map<String, dynamic>>> robleRead(
    ...
    final resolved = await _resolveTableName(tableName);
    return roble.read(resolved, filters: filters);
  }
  ...
  Future<Map<String, dynamic>> robleUpdate(
    ...
    final resolved = await _resolveTableName(tableName);
    return roble.update(resolved, id, updates);
  }
