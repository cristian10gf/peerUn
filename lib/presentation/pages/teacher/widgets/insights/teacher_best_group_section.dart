...
class TeacherBestGroupSection extends StatelessWidget {
  final List<TeacherInsightsBestGroupVm> items;

  const TeacherBestGroupSection({super.key, required this.items});
      ...
      children: [
        ...
        if (items.isEmpty)
          const _SectionEmptyCard(
            message: 'Aun no hay grupos con muestras minimas',
          )
        else
          ...items.map((item) => _BestGroupCard(item: item)),
      ],
    );
  }
}

class _BestGroupCard extends StatelessWidget {
  final TeacherInsightsBestGroupVm item;

  const _BestGroupCard({required this.item});
